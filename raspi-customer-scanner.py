...
def sniffmgmt(p):
    ...
    #if p.haslayer(Dot11): this seems to be buggy at the moment, but I don't know the function anyway, just changed
    # to always true
    if True:
        # Check to make sure this is a management frame (type=0) and that
        # the subtype is one of our management frame subtypes indicating a
        # a wireless client
        print("MAC: {:s}".format(str(p.addr2)))
        print("Type {:d} and subtype {:d}".format(p.type, p.subtype))
        print("")
...
print("")
sniff(iface=interface, prn=sniffmgmt, timeout=10)
print("Stop sniffing")
print("Detected {:d} clients".format(len(observedclients)))